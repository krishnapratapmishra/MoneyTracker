...
| 2026-05-11 | 3.0 | GTT Signal Table: `api_trading_strategy` rewritten to use correct 3-table JOIN (assets → AssetMapping → InvestMapping + LEFT JOIN nse_master); all column refs updated to new schema (avgprice, investedvalue, currentvalue, targetpct, lastsynced, am.AssetSymbol, im.AssetType); filter changed from old `asset_type` col to `im.AssetType / im.AssetClass`; "Unexpected token '<'" error resolved |
| 2026-05-11 | 2.9 | Portfolio drill-down (`wtShowDrilldown`): now groups assets by `asset_category` and renders `_wtCategoryCard` (summary per category with totals + individual asset rows inline); Growth asset class filters to only unsynced assets (`!a.last_synced`); `wtSaveLtp` + `wtDeleteAsset` updated to re-render the whole drill-down via `_wtDrillClass()` helper instead of trying to update individual `wtac-N` elements that no longer exist |
| 2026-05-11 | 2.8 | Portfolio + Loans top cards: `#portfolioTopCards` and `#loanTopCards` override `cards-grid` with `repeat(4,1fr)` — all 4 cards fill full row width; Wealth Goal cards: `wt-goals-grid` changed from flex-scroll to CSS grid with `--wg-cols` custom property set to `goals.length` (capped 1–6); responsive breakpoints at 900px (3 cols) and 600px (2 cols) |
| 2026-05-11 | 2.7 | Universe at a Glance stat cards: fixed `panel.style.display=''` (deferred to CSS `display:none`) → `'block'`; all 4 cards (Vision Cards, Status Metrics, Life Areas, Total Intentions) now show detail panel on click; Live Market tabs: replaced 30-line `!important` CSS block with `_mwApplyTabStyle()` inline-style approach; fixed tabs get colors from `_MW_FIXED_COLORS`; dynamic tabs cycle through 15-color `_MW_DYN_PALETTE`; `_buildMwTabs` assigns `data-color` + applies inactive style immediately |
| 2026-05-11 | 2.6 | Hub tab font-weight: reduced from 700 (bold) to 500 (medium) inactive, 600 active — prevents blurry/heavy appearance; WCE category card backgrounds changed to deep gradient (dark indigo/amber/emerald/navy/crimson/blue); all in-card text, ring labels, subtext changed to white-based colors; ring track strokes → `rgba(255,255,255,.18)`; card borders removed, replaced with dark `box-shadow`; ALLOC ring fixed to show `allocAchieved = actPct/tgtPct×100` (progress toward target) instead of raw actPct — prevents ALLOC and INV SHARE showing same value |
